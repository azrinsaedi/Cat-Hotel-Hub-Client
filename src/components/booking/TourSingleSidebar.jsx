...
export const action = async ({ request }) => {
  ...
  try {
    await customFetch.post("/account/add-booking", data);
...
export default function TourSingleSidebar({ data }) {
  const { cats } = data.customer;
  const { _id } = data.hotel;
  ...
  const handleCheckboxChange = (index, catId) => {
    const newCheckedItems = [...checkedItems];
    newCheckedItems[index] = !newCheckedItems[index];

    setCheckedItems(newCheckedItems);

    // Update selectedCatIds based on checkedItems
  ...
  // console.log("dates[0]", dates[0].toDate());
  // console.log("dates[1]", dates[1].toDate());
  ...
  return (
    <div className="tourSingleSidebar">
      <Form method="post">
        <input type="hidden" id="check_in" name="check_in" value={dates[0]} />
        <input type="hidden" id="check_out" name="check_out" value={dates[1]} />
        <input
          type="hidden"
          id="occupied_rooms"
          name="occupied_rooms"
        ...
        <input
          ...
          required
        />
        <input type="hidden" id="hotel" name="hotel" value={_id} required />
                ...
                <div className="searchFormItem__content">
                  ...
                  <div>
                    <span className="js-first-date">
                      <Calender dates={dates} setDates={handleDateChange} />
                    </span>
        ...
        {cats.map((elm, i) => {
          return (
            <div key={i} className="d-flex items-center justify-between">
              <div className="d-flex items-center">
                <div className="form-checkbox">
                  <input
                    onChange={() => handleCheckboxChange(i, elm._id)}
                    type="checkbox"
                ...
                <div className="ml-10">{elm.name}</div>
              </div>
      ...
      <div className="d-flex justify-between mt-20">
        ...
        {/* Input field to store selectedCatIds */}
        <input
          type="hidden"
          name="cats"
          value={JSON.stringify(selectedCatIds)}
        />
        {/* <Link to="/booking"> */}
